defmodule DoctorSchedule.Appointments.Repositories.ProviderRepositoryTest do
  ...
  describe "Tests Provider in Appointments" do
    ...
    test "all_day_from_provider/2 returns all appointments from provider in a month" do
      appointment =
        AppointmentFixture.appointment_fixture()
      appointment =
        appointment
      ...
      date =
        NaiveDateTime.to_date(appointment.date)

      assert ProviderRepository.all_day_from_provider(appointment.provider.id, date)
             |> Enum.count() == 1
    end
  end
end
